...
I found blog posts that referenced using `@next/mdx`, then `next-mdx-enhanced`, but there were performance problems, so the latest recommendation was to use `next-mdx-remote`.

I found a quick hack by looking at vercel's [example code](https://github.com/vercel/next.js/blob/canary/examples/with-mdx-remote
